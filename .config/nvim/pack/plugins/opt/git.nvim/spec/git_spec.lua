...
describe(':Gdiff', function()
	...
	it('shows staged diff by default', function()
		vim.cmd.edit('file')
		vim.cmd.Gdiff()
	...
	it('shows diff against specific commit', function()
		vim.cmd.edit('file')
		vim.cmd.Gdiff('@~4')
		vim.cmd.wincmd('p')
...
describe(':Gblame', function()
	...
	it('auto-previews commit', function()
		git_config_user()
		mkfile('a', { 'a', 'a', 'a' })
		git_add()
		git_commit()
...
describe(':Glog', function()
	it('edits git-log://', function()
		...
		vim.cmd.Glog('@~10 -n100')
		assert.same('git-log://@~10 -n100', vim.fn.bufname())
	end)
end)
...
describe('git-log://', function()
	...
	it('uses log when log-vim not found', function()
		vim.cmd.edit('git-log://')
		wait()
		local lines = vim.api.nvim_buf_get_lines(0, 0, -1, false)
		assert.matches('^commit', lines[1])
		assert.matches('Initial commit', lines[#lines])
	end)

	it('reads log-vim with the given arguments', function()
		git_log_vim()
		git('switch', '-C', 'test')
...
describe('<Plug>(git-goto-file)', function()
	...
	it('falls back to gf for non-git objects', function()
		vim:set_lines({ 'a/b/c' })
		vim:feed('s')
