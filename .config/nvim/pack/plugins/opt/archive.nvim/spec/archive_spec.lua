...
describe('zip', function()
	...
	local function edit_deleted_archive()
		it(":edit errors out when archive doesn't exist", function()
			vim.fn.delete(zip_path)
			assert.error_matches(vim.cmd.edit, 'zip.*:.*cannot find or open')
			vim:assert_lines({ '' })
	...
	before_each(function()
		...
		end, 'zipinfo:.*cannot find or open')
	...
	describe('list view', function()
		...
		it(':edit reloads members', function()
			...
			vim.cmd.edit()
			vim:assert_lines({ c_path:sub(2), foo_path:sub(2) })
		end)
...
describe('xz', function()
	...
	it(':edit errors out when file is not readable', function()
		vim.fn.setfperm(xz_path, '---------')
		assert.error_matches(vim.cmd.edit, 'xz:.*Permission denied')
		vim:assert_lines({ '' })
		assert.same('nofile', vim.bo.buftype)
		assert.same('', vim.bo.filetype)
		assert.False(vim.bo.modeline)
	...
	it(":edit doesn't error out when file doesn't exist", function()
		vim.fn.delete(xz_path)
		vim.cmd.edit()
		vim:assert_lines({ '' })
		assert.same('c', vim.bo.filetype)
	...
	it(':write writes compressed file', function()
		...
		vim:assert_messages(('"%s" written with xz'):format(xz_path))
	end)
end)
